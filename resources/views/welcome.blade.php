...
<body class="font-sans antialiased bg-gray-900 text-white min-h-screen flex flex-col">
    ...
    <main class="flex-1 flex items-center justify-center px-8">
        <div class="text-center max-w-2xl">
            <div class="inline-flex items-center px-3 py-1 rounded-full bg-indigo-600/20 border border-indigo-500/30 text-indigo-400 text-xs font-medium mb-6">
                v{{ config('opterius.version', '1.0.0') }}
            </div>

            <h1 class="text-5xl sm:text-6xl font-bold leading-tight">
                <span class="text-white">Opterius</span>
                <span class="text-indigo-500">Panel</span>
            </h1>
            ...
            <div class="mt-10 flex items-center justify-center space-x-4">
                @auth
                    <a href="{{ route('dashboard') }}"
                       class="inline-flex items-center px-8 py-3.5 bg-indigo-600 text-white text-sm font-semibold rounded-xl hover:bg-indigo-700 transition shadow-lg shadow-indigo-500/25">
                        Go to Dashboard
