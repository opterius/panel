<x-user-layout>
    <x-slot name="header">
        <h2 class="text-lg font-semibold text-gray-800">{{ __('cron.cron_jobs') }}</h2>
    </x-slot>
    ...
    <div class="bg-white rounded-xl shadow-sm">
        <div class="flex justify-between items-center px-6 py-5 border-b border-gray-100">
            <div>
                <h3 class="text-base font-semibold text-gray-800">{{ __('cron.cron_jobs') }}</h3>
                <p class="text-sm text-gray-500 mt-1">{{ __('cron.schedule_commands') }}</p>
            </div>
        ...
        @if($cronJobs->isEmpty())
            <div class="px-6 py-16 text-center">
                ...
                <h3 class="mt-4 text-base font-medium text-gray-700">{{ __('cron.no_cron_jobs_yet') }}</h3>
                <p class="mt-2 text-sm text-gray-500">{{ __('cron.schedule_your_first_task') }}</p>
                    ...
                    <div class="flex items-center justify-between px-6 py-4 hover:bg-gray-100 transition">
                        ...
                        <div class="flex items-center space-x-3">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium
                                ...
                                {{ $cron->enabled ? __('common.active') : __('common.disabled') }}
                            </span>
                            ...
                            <form action="{{ route('user.cronjobs.toggle', $cron) }}" method="POST">
                                ...
                                <button type="submit" class="text-sm font-medium transition
                                    ...
                                    {{ $cron->enabled ? __('common.disable') : __('common.enable') }}
                                </button>
                            </form>

                            <x-delete-modal
                                :action="route('user.cronjobs.destroy', $cron)"
                                :title="__('cron.delete_cron_job')"
                                :message="__('cron.delete_cron_confirmation')"
                                :confirm-password="true">
                                <x-slot name="trigger">
                                    <button type="button" class="text-gray-400 hover:text-red-600 transition" title="{{ __('common.delete') }}">
