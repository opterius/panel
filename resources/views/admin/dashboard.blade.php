<x-admin-layout>
    <x-slot name="header">
        <h2 class="text-lg font-semibold text-gray-800">{{ __('dashboard.server_dashboard') }}</h2>
    </x-slot>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="bg-white rounded-xl shadow-sm p-6 flex items-center space-x-4">
            ...
            <div>
                <div class="text-sm font-medium text-gray-500">{{ __('dashboard.servers') }}</div>
    ...
    <div class="bg-white rounded-xl shadow-sm">
        <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center">
            <h3 class="text-base font-semibold text-gray-800">{{ __('dashboard.servers') }}</h3>
        ...
        @if($servers->isEmpty())
            <div class="px-6 py-12 text-center">
                <p class="text-sm text-gray-500">{{ __('dashboard.no_servers_yet') }}</p>
            </div>
        @else
                    ...
                    <a href="{{ route('admin.servers.show', $server) }}" class="flex items-center justify-between px-6 py-4 hover:bg-gray-100 transition">
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <div class="text-xs text-gray-500">{{ $server->ip_address }} &middot; {{ $server->domains->count() }} {{ __('dashboard.domains') }} &middot; {{ $server->accounts->count() }} {{ __('dashboard.accounts') }}</div>
                            </div>
                        </div>
