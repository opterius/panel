    ...
    @if($atLimit)
        <div class="mb-6 bg-amber-50 border border-amber-200 text-amber-700 px-4 py-3 rounded-lg text-sm flex items-center justify-between">
            <div class="flex items-center space-x-2">
                ...
                <span>{{ __('accounts.upgrade_limit_banner', ['current' => $currentAccounts, 'max' => $maxAccounts]) }} <a href="https://github.com/siyamex/panel" target="_blank" class="underline font-medium">{{ __('accounts.upgrade_your_license') }}</a> {{ __('accounts.to_create_more_accounts') }}</span>
            </div>
        </div>
    @endif
