<x-user-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-lg font-semibold text-gray-800">PostgreSQL Databases</h2>
            <a href="{{ route('user.postgres.create') }}"
    ...
    <div class="bg-white rounded-xl shadow-sm">
        <div class="flex justify-between items-center px-6 py-5 border-b border-gray-100">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">PostgreSQL databases for your hosting accounts.</p>
            </div>
        </div>
        ...
        @if($databases->isEmpty())
            <div class="px-6 py-16 text-center">
                ...
                <h3 class="text-base font-medium text-gray-700">No PostgreSQL databases yet</h3>
                <p class="mt-2 text-sm text-gray-500">Create your first PostgreSQL database to get started.</p>
                <a href="{{ route('user.postgres.create') }}"
                    ...
                    <div class="flex items-center justify-between px-6 py-4 hover:bg-gray-50 transition">
                        <div class="flex items-center gap-4">
                            <div class="w-10 h-10 rounded-lg flex items-center justify-center
                                @if($db->status === 'active') bg-blue-100 @else bg-red-100 @endif">
                                <svg class="w-5 h-5 @if($db->status === 'active') text-blue-600 @else text-red-600 @endif"
                                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4"/>
                                </svg>
                            </div>
                            <div>
                                <a href="{{ route('user.postgres.show', $db) }}"
                                   class="text-sm font-semibold text-indigo-600 hover:text-indigo-800 transition">{{ $db->name }}</a>
                                <div class="text-xs text-gray-400 mt-0.5">
                                    User: <span class="font-mono">{{ $db->pg_username }}</span>
                                    &middot; {{ $db->account->server->name }}
                                    &middot; {{ $db->account->username }}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <x-delete-modal
                                ...
                                :message="'Permanently delete &quot;' . $db->name . '&quot; and its user? This cannot be undone.'"
